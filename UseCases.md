...
[<img src="pics/UML_cases.jpg" width="600"/>]()
...
| Прецедент использования | Добавление общих задач |
| :---: | :---: | 
| Действующие лица | Администратор | 
| Краткое описание | Позволяет добавить задачу, которую увидят в планере все акторы | 
| Предусловия | Желание создать напоминание, которое будет у всех |
| Основной поток | Администратор заполняет форму задачи, указывая название задачи, краткое описание(необязательно), дату или крайний срок выполнения | 
| Альтернативные потоки | Администартор не указал одно из обязательных полей, должен либо повторно заполнить форму, либо отменить создание задачи |
| Постусловия | Если прецедент использования завершился успешно, в планере появляется новая задача, в противном случае состояние системы остается неизменным |

| Прецедент использования | Удаление общих задач |
| :---: | :---: | 
| Действующие лица | Администратор | 
| Краткое описание | Позволяет удалить задачу | 
| Предусловия | Желание убрать задачу, отображаемую у всех |
| Основной поток | Администратор выбирает задачу и подтверждает ее удаление | 
| Альтернативные потоки | Администратор не подтвердил удаление, должен либо подтвердить удаление, либо отменить удаление |
| Постусловия | Если прецедент использования завершился успешно, из планера пропадает задача, в противном случае состояние системы остается неизменным |

| Прецедент использования | Редактирование учетных записей |
| :---: | :---: | 
| Действующие лица | Администратор | 
| Краткое описание | Позволяет отредактировать каталог учетных записей | 
| Предусловия | Необходимость внести изменения в учетные записи |
| Основной поток | Администратор выбирает режим редактирования, вносит изменения, после чего сохраняет их | 
| Альтернативные потоки | Если формат измененных данных не соответствует требованиям, необходимо внести правки вновь, либо отменить изменения |
| Постусловия | Если прецедент использования завершился успешно, в базе данных и в приложении меняется информация, в противном случае состояние системы остается неизменным |
